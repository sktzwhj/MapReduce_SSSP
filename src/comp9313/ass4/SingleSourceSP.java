...
public class SingleSourceSP {
	...
	public static class SPMapper extends Mapper<Object, Text, IntWritable, Text> {
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			while (itr.hasMoreTokens()) {
				...
				/*
				 * According to our format transformation, the original input with format [id, fromNode, toNode, distance] is transformed 
				 * into format of [nodeId distance| adjacentList(nodeId:edgeLength) |updated(Y/N) ]
				 */
				...
				/*
				 *  O for original distance, the reducer can use this to know whether the shortest distance for a certain node 
				 *  has been updated.
				 */
				...
				if ((Double.compare(shortestDistance, maxDistance) != 0) && nodeInfo[1].length()>1) {
					...
					for (int i = 0; i < adjacentNodes.length; i++) {
						...
						//the condition is to filter some exceptions, i.e., node 240 has no adjacentList. 
	...
	public static class SPReducer extends Reducer<IntWritable, Text, Text, Text> {
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context)
			...
			for (Text val : values) {
				...
				try{
					if (val.toString().startsWith("O")) {
						//get the original shortest distance in the last run for this node
					...
					// get the adjacent list which needs to be appended in the output. 
			...
			if (Double.compare(originalDistance, minShortestDistance) != 0) {
				...
				update = true;
				//we use updateCounter to see if there is some update in this run of mapreduce job.
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * It seems that we cannot specify the S3 url as the input for the read operations in main function (though we can do that in mapper). 
		 * Therefore, we use the AWS API to do this.  
		 */
		AWSCredentials credentials = new BasicAWSCredentials("AKIAIZ5IA2UP27LMYL6A","PSclr7E9LujwpFd8/g5ILusfy+9dqUsuHTTbgmzW");
		
		//We firstly transform the file format 
		AmazonS3Client s3Client = new AmazonS3Client(credentials);
		
		S3Object s3object = s3Client.getObject(new GetObjectRequest("comp9313.z5055605",String.format("problem2/%s", args[0])));
		...
		while ((lineContent = inputBuffer.readLine()) != null) {
			//filtering out the first several lines which starts with #
			if (!lineContent.startsWith("#")){
				...
				if (Integer.parseInt(edgeRecord[1]) != curSourceNode) {
					...
					//Actually, Y or N is not used in the program, but it can be used to see if the distance has been updated in last run
					tmpNewFmtRecord += "|Y\n";

					newFormatInput.writeBytes(tmpNewFmtRecord);

					curSourceNode = Integer.parseInt(edgeRecord[1]);
					//the shortest distance for the source node is 0.0
					if (Integer.parseInt(edgeRecord[1]) == singleSource){
						...
						tmpNewFmtRecord = edgeRecord[1] + " " + maxDistance.toString() + "|" + edgeRecord[2] + ":"
							+ edgeRecord[3].toString() + ",";
					}
		...
		// to fix the problem of last line.
		tmpNewFmtRecord += "|Y";
